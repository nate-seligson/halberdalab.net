...
var busy = false, meansSetting, scatterFrameNum = 0, curScatterFrameQueue, isPlayingScatter = false, graphExportCurAge, lastGraphDataNum; //states
	...
	if(plotConstraints.popMean == true) {
		...
		for(i = 0; i < datasetOvalCenter.length; i++) {
			if(checkPointValidityOval(datasetOvalCenter[i].m, datasetOvalCenter[i].a)) {
				...
				if (datasetOvalCenter[i].d[0][0] == datasetOvalCenter[i].d[1][0]) {
					tempData.data.push([ datasetOvalCenter[i].d[0][0]/1000 , datasetOvalCenter[i].d[0][1] - 4 ]);
					tempData.data.push([ datasetOvalCenter[i].d[1][0]/1000 , datasetOvalCenter[i].d[1][1] + 4 ]);
				}else {
					tempData.data.push([ (datasetOvalCenter[i].d[0][0] - 1 ) / 1000, datasetOvalCenter[i].d[0][1]]);
					tempData.data.push([ (datasetOvalCenter[i].d[1][0] + 1 ) / 1000, datasetOvalCenter[i].d[1][1]]);
				}
		...
		for(i = 0; i < datasetOvalGray.length; i++) {
			if(checkPointValidityOval(datasetOvalGray[i].m, datasetOvalGray[i].a)) {
				...
				if (datasetOvalGray[i].d[0][0] == datasetOvalGray[i].d[1][0]) {
					tempData.data.push([ datasetOvalGray[i].d[0][0]/1000 , datasetOvalGray[i].d[0][1] - 8 ]);
					tempData.data.push([ datasetOvalGray[i].d[1][0]/1000 , datasetOvalGray[i].d[1][1] + 8 ]);
				}else {
					tempData.data.push([ (datasetOvalGray[i].d[0][0] - 2 ) / 1000, datasetOvalGray[i].d[0][1]]);
					tempData.data.push([ (datasetOvalGray[i].d[1][0] + 2 ) / 1000, datasetOvalGray[i].d[1][1]]);
				}
...
function generateMovieForDownload() {
	...
	$.get('retrieveMovie.php?checkExists=1&fileID='+fileID, function(response) {
		if(response != "not found") {
			...
			$('#subHeading').html('Uploading data (' + Math.round(imageData.length/1000000) + 'MB total).  This may take a few minutes. <img src="images/spinner.gif"/>');
			$.post("processVideo.php", { "fileID": fileID, "delay": ($('#cbShowPopMean').is(':checked') ? "3" : "10"), "imageData": imageData }).complete(function() {
				$.get('retrieveMovie.php?fileID=' + fileID, function(response) {
...
function loadScatterPreset(autoPlay, sliderValue) {
	...
	var prevModalText = $('.modalMsg').html();
	$('#subHeading').html('Loading data from 13,577 individuals... <img src="images/spinner.gif"/>');
	$('.modalMsg').html('<h3>Loading...</h3>');
	$('.modal').show();
	
	$LAB.script("js/data.preset1.js?_"+Math.random()).wait(function() {
		dataURLlist = preset1;
		dataURLlistWMeans = ovalpreset1;
		$('#subHeading').html(prevSubText);
		$('.modalMsg').html(prevModalText);
		$('.modal').hide();
		if(autoPlay) {
			$('#btnPlayScatter').trigger('click');
		} else {
