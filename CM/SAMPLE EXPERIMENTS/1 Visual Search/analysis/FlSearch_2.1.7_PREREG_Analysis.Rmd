...
expName = "FlSearch fire 2.1.7 Prereg"
exclusionCriteria <- .8
...
p <- ggplot(datad, aes(x= set_size, y=medianRT, fill=target2)) +
    geom_line(position=position_dodge(.9), colour="white", stat="identity") +
...
p3 <- ggplot(datad, aes(x= set_size, y=medianRT, color=target2, group = target2))+
  geom_point() + 
    geom_errorbar(position=position_dodge(.0), width=.4, size = 3, aes(ymin=medianRT-ci, ymax=medianRT+ci)) +
  geom_smooth(method = "loess", se = FALSE, size = 3) +
#geom_errorbar(position=position_dodge(.0), width=.25, aes(ymin=lookingtime-se, ymax=meanLookingTime+se)) +
    ggtitle(paste0(" ")) +
   scale_color_manual(values=c("#DC965A","#980D00"),labels = c("Fire burns up (normally)", "Fire burns down (reverse)")) +
   labs(x = "Set size", y = "Median RT (ms)", color = "Legend Title\n") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme_Publication() +
     theme(legend.position="none")
p3
ggsave(file="ZPlot.png", plot=p3, width=12, height=5)


p3 <- grid.arrange((p3 + scale_fill_Publication() +theme_Publication() +scale_fill_manual(values=c("#980D00","#c8945c"),labels = c("Fire burns up (normally)", "Fire burns down (reverse)"))),nrow=1)
#ggsave(file="ZPlot.png", plot=p3, width=10, height=8)
...
p3 <- ggplot(datad, aes(x= set_size, y=meanAcc, color=target, group = target))+
  geom_point() + 
    geom_errorbar(position=position_dodge(.0), width=.4, size = 3, aes(ymin=meanAcc-ci, ymax=meanAcc+ci)) +
  geom_smooth(method = "loess", se = FALSE, size = 3) +
#geom_errorbar(position=position_dodge(.0), width=.25, aes(ymin=lookingtime-se, ymax=meanLookingTime+se)) +
    ggtitle(paste0(" ")) +
   scale_color_manual(values=c("#980D00","#DC965A"),labels = c("Fire burns up (normally)", "Fire burns down (reverse)")) +
   labs(x = "Set size", y = "Percent correct", color = "Legend Title\n") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme_Publication() +
     theme(legend.position="none")
p3
ggsave(file="acc.png", plot=p3, width=12, height=5)

p3

p3 <- grid.arrange((p3 + scale_fill_Publication() +theme_Publication() +scale_fill_manual(values=c("#980D00","#c8945c"),labels = c("Fire burns up (normally)", "Fire burns down (reverse)"))),nrow=1)
#ggsave(file="acc.png", plot=p3, width=10, height=8)
