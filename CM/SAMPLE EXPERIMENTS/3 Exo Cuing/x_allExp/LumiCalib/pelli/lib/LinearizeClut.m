...
checkLuminance=1;       % optional diagnostic print out
checkLinearization=1;   % optional diagnostic print out


cal.nFirst=1;                                   % NMH 2020_04
cal.nLast=255; %size(cal.old.gamma,1)-1;
...
cal.old.L_original = cal.old.L;                 % NMH 2020_04
if size(cal.old.L_original,1) == 4
    cal.old.gamma = [cal.old.gamma,cal.old.gamma(:,2)];
end


for idx_c = 1:size(cal.old.L,1)                 % NMH 2020_04
    cal.old.L = cal.old.L_original(idx_c,:);    % NMH 2020_04
    
    cal.LFirst=min(cal.old.L);                  % NMH 2020_04
    ...
    if size(cal.old.L_original,1) == 1      % NMH 2020_04
        c2check = cal.old.gamma(:,2);
        numCol = 3;
    else
        c2check = cal.old.gamma(:,idx_c);
        numCol = 4;
    end
    
    if ~all(diff(c2check)>0)
        % If necessary, force monotonicity of the green channel of the old gamma table.
        g=c2check;  % NMH 2020_04
        oldG=g;
    ...
    % Interpolate index into cal.old.gamma
    index=interp1(c2check,1:length(cal.old.gamma),cal.G,'pchip'); % (takes few ms) % NMH 2020_04
    % Interpolate RGB color (a white triplet) at the index of the luminance we want.
    
    if size(cal.old.L_original,1) == 1
        linearizedGamma=interp1(cal.old.gamma,index,'pchip'); % (takes few ms)
    else
        linearizedGamma=interp1(cal.old.gamma(:,idx_c),index,'pchip')'; % (takes few ms)
    end
    linearizedGamma=min(linearizedGamma,1);
    linearizedGamma=max(linearizedGamma,0);
        
    if ~isfield(cal,'gamma') %|| (size(cal.old.L_original,1) >1 && size(cal.gamma,2) < 3) %% NMH 2020_04
        ...
        cal.gamma=ones(cal.clutMapLength,numCol); %% NMH 2020_04
        if numCol <= 3
            cal.gamma=cal.old.gamma(round(1+oldMaxIndex*(0:maxIndex)/maxIndex),1:numCol); %% NMH 2020_04
        else
            cal.gamma=cal.old.gamma(round(1+oldMaxIndex*(0:maxIndex)/maxIndex),1:numCol);
        end
    ...
    if size(cal.old.L_original,1) == 1 %% NMH 2020_04
        ...
        cal.gamma(1+cal.n,idx_c)=linearizedGamma;
    end
    
    
    
    if checkLinearization && (size(cal.old.L_original,1) == 1 || idx_c == 4)
        %err=rms(linearizedGamma(:,2)'-cal.G);  
        if size(cal.old.L_original,1) == 1          %% NMH 2020_04
            err=rms(linearizedGamma(:,2)'-cal.G);
        else
            err=rms(linearizedGamma(:,1)'-cal.G);
        end
    ...
    cal.L_all(idx_c,:) = cal.L;
end

cal.L = cal.L_all; %% NMH 2020_04
if size(cal.old.L_original,1) == 4
    cal.old.gamma = cal.old.gamma(:,1:3);
    cal.gamma_rgba = cal.gamma;
end
cal.gamma = cal.gamma(:,1:3);
