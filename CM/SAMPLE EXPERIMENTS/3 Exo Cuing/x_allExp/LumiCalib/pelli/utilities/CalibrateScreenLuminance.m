...
forceMaximumBrightness=false; %%% NMH 2020_04
...
%         luminances=64+1;
    ...
    if IsOSX %|| IsLinux
        ...
        if ~makeItQuickForDebugging
            ...
            useBrightness=true; %%% NMH 2020_04
            if useBrightness
    ...
    try
        ...
        if strcmp(caliMode,'gray') %%% NMH 2020_04
            cal.old.L=zeros(1,luminances);
            channels = 1;
        elseif strcmp(caliMode,'rgba')
            cal.old.L=zeros(3,luminances);
            channels = 1:4;
        end
        
        for idx_c = channels %%% NMH 2020_04
            i = 0;
        while i<luminances
            ...
            cal.old.G(i)=cal.old.gamma(round(1+cal.old.n(i)*cal.old.gammaIndexMax/cal.pixelMax),2);         %%% cal.old.G
            v=white*cal.old.n(i)/cal.pixelMax;
            
            % display the test color
            if strcmp(caliMode,'rgba')       %% NMH 2020_4
                switch idx_c
                    case 1; v = [v,0,0]; % red
                    case 2; v = [0,v,0]; % green
                    case 3; v = [0,0,v]; % blue
                    case 4; v = [v,v,v]; % all   
                end
            end
            ...
            if useConnectedPhotometer
                ...
                n=GetLuminance;
                if useEV
                    cal.old.L(idx_c,i)=2^(n-3); % Convert EV to cd/m^2, assuming film speed is ISO 100.
                else
                    cal.old.L(idx_c,i)=n;
                end
            else
            ...
            if 1
                ...
                fprintf('i %3d, n %3d, v %.2f, L %5.1f cd/m^2\n',i,cal.old.n(i),max(v),cal.old.L(idx_c,i)); %% NMH 2020_4
            end
        end
        end
    catch e
    ...
    for f=[1,fid]
        if ~isempty(cal.MACAddress)
            ...
            fprintf(f,'if streq(MACAddress,''%s'') && streq(const.calibType,''%s'')\n',cal.MACAddress,caliMode);  %%% NMH 2020_04
            fprintf(f,'\t%% ');
        end
        fprintf(f,'if ');
        if IsWin
        ...
        fprintf(f,'\tcal.caliMode=''%s'';\n',caliMode); %%% NMH 2020_04
        ...
        %fprintf(f,' %g',cal.old.L);%% NMH 2020_04
        for i=1:size(cal.old.L,1)
            for ii=1:size(cal.old.L,2)
                if ii ==size(cal.old.L,2)
                    fprintf(f,'%.6f;',cal.old.L(i,ii));
                else
                    fprintf(f,'%.6f ',cal.old.L(i,ii));
                end
            end
            if i < size(cal.old.L,1)
            fprintf(f,'...\n');
            end
        end
...
%     cal.LLast=max(cal.old.L);
    ...
    for idx_c = 1:size(cal.old.L,1)             %% NMH 2020_04
        ...
        subplot(1,size(cal.old.L,1),idx_c);     %% NMH 2020_04
        
        plot(cal.old.G,cal.old.L(idx_c,:));     %% NMH 2020_04
        xlabel('Gamma value');
        ...
        if any(diff(cal.old.L(idx_c,:))<0)      %% NMH 2020_04
            warning('The plotted luminance function (Fig. 1.%i) is not monotonic.',idx_c); %% NMH 2020_04
        end
