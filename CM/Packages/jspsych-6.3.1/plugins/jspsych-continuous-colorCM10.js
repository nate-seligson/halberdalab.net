...
jsPsych.plugins['continuous-colorCM10'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-colorCM10',
        description: '',
        parameters: {
            set_size: {
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        var width2 = trial.radius * 2 + trial.item_size * 2 + 50;
        var height2 = trial.radius * 2 + trial.item_size * 2 + 50;
        var center = width/2;
        var center2 = center - 420;
        var center3 = center + 420;
...
//line 304 width: controls fix cross loc
/* OLD
html += `<span id="contMemoryFixation" style="cursor: pointer; z-index: 100; left: -400px;width: 10px; ">+</span>
<div id="contMemoryStartTrial" style="position: absolute; 
    top:50px;">${startText}</div>
<div id="item-1" class="contMemoryItem" 
    style="display: none; top:${center-trial.item_size/2}px; left:${center-trial.item_size/2}px; 
width:${trial.item_size}px; 
height:${trial.item_size}px; "></div>
<div id="reportDiv" style = "width: 400px; height: 50px; top:${center-trial.item_size/2}px; left:-400px;"   ></div>
</div>`;
display_element.innerHTML = html;
*/  

        html += `<span id="contMemoryFixation" style="cursor: pointer; z-index: 100; left: 0px;width: 5000px; "></span>
             <div id="contMemoryStartTrial" style="position: absolute; 
                 top:50px;">${startText}</div>
             <div id="item-1" class="contMemoryItem" 
                 style="display: none; 
         width:${trial.item_size}px; 
         height:${trial.item_size}px; "></div>
             <div id="reportDiv" style = "width: 1000px; height: 0px; cursor: pointer; top:${center-trial.item_size/2}px; left:${center3-trial.item_size/2}px;"   >+</div>
         </div>`;
        display_element.innerHTML = html;
       ...
       // displayWheel();
        ...
        var startTrial = function() {
          //  removeEventListener('click', startTrial);
            ...
            if (trial.click_to_start) {
                ...
                document.getElementById("reportDiv").innerHTML = '<font size="145">+</font>';
                document.getElementById('contMemoryFixation').style.cursor = 'none';
            }
            document.getElementById('contMemoryFixation').style.cursor = 'none';
        ...
        if (trial.click_to_start) {
            display_element.querySelector('#reportDiv').addEventListener('click', startTrial);
...
//end
            ...
            for (var i=0; i<wheelOptions.length; i++) {
                ...
                var leftPx = center3-10 + wheel_radius * Math.cos(positionDeg/180.0*Math.PI);    
