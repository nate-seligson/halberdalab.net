...
jsPsych.plugins['many-guess-4'] = (function() {
...
for (var i = 0; i < totalCircles; i++) {
  var curRadius = Math.random() < 0.5 ? radius1 : radius2;
  var numPlaceholders = Math.floor(2 * Math.PI * curRadius / (trial.item_size/2));
  var angle = Math.random() * 2 * Math.PI;
  var curTop = (Math.cos(angle) * curRadius) - trial.item_size / 2 + center;
  var curLeft = (Math.sin(angle) * curRadius) - trial.item_size / 2 + center;
  html += `
    <div id="item${i}" class="contMemoryItem" 
      style="top:${curTop}px; left:${curLeft}px; background-color:${trial.bg_color};
      width:${trial.item_size}px; height:${trial.item_size}px">
    </div>`;
  possiblePositions.push(i);
}
...
var innerRadius = 100;//trial.radius2;
var itemSize = trial.item_size;
var outerCircumference = 2 * Math.PI * outerRadius;
var outerCircleNumItems = Math.round(outerCircumference / itemSize);
var innerCircumference = 2 * Math.PI * innerRadius;
var innerCircleNumItems = Math.round(innerCircumference / itemSize);

// Generate random positions for each circle
var outerPositions = [];
var innerPositions = [];
for (var i = 0; i < outerCircleNumItems; i++) {
  var angle = (2 * Math.PI / outerCircleNumItems) * i;
  var x = Math.round(outerRadius * Math.cos(angle));
  var y = Math.round(outerRadius * Math.sin(angle));
  outerPositions.push({x: x, y: y});
}
for (var i = 0; i < innerCircleNumItems; i++) {
  var angle = (2 * Math.PI / innerCircleNumItems) * i;
  var x = Math.round(innerRadius * Math.cos(angle));
  var y = Math.round(innerRadius * Math.sin(angle));
  innerPositions.push({x: x, y: y});
}

// Combine the positions into a single array
var positions = [];
for (var i = 0; i < outerPositions.length; i++) {
  positions.push({x: outerPositions[i].x + center, y: outerPositions[i].y + center});
}
for (var i = 0; i < innerPositions.length; i++) {
  positions.push({x: innerPositions[i].x + center, y: innerPositions[i].y + center});
}
...
for (var i = 0; i < positions.length; i++) {
  html += `<div id="item${i}" class="contMemoryItem" 
             style="top:${positions[i].y - itemSize/2}px; left:${positions[i].x - itemSize/2}px; 
