...
jsPsych.plugins["video-keyboard-responseCM"] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('video-keyboard-responseCM', 'stimulus', 'video');

  plugin.info = {
    name: 'video-keyboard-responseCM',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // setup stimulus
    var video_html = '<div>'
    video_html += '<video id="jspsych-video-keyboard-responseCM-stimulus"';
    ...
    if(!video_preload_blob) {
      for(var i=0; i<trial.stimulus.length; i++){
        ...
        if (type == "mov") {
          console.warn('Warning: video-keyboard-responseCM plugin does not reliably support .mov files.')
        }
    ...
    var video_element = display_element.querySelector('#jspsych-video-keyboard-responseCM-stimulus');
    ...
    function end_trial() {
      ...
      display_element.querySelector('#jspsych-video-keyboard-responseCM-stimulus').pause();
      display_element.querySelector('#jspsych-video-keyboard-responseCM-stimulus').onended = function(){ };
    ...
    var after_response = function(info) {
      ...
      display_element.querySelector('#jspsych-video-keyboard-responseCM-stimulus').className += ' responded';
      ...
      if (trial.response_ends_trial && info.key == 'j') {

        if (info.rt > 20000) {
    
        } else if (info.rt < 20000){
          end_trial();
        }
