...
jsPsych.plugins['continuous-orientation-circle10'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-orientation-circle10',
        description: '',
        parameters: {
            set_size: {
       ...
       #contMemoryBox {
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            html += `<div id="item${i}" class="contMemoryItem" 
                 style="top:${curTop}px; transform: rotate(${0}deg); left:${curLeft}px; background-color: '';
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            html += `<div id="item2${i}" class="contMemoryItem" 
                 style="top:${curTop}px; transform: rotate(${trial.mask_rotation[0][i]}deg); left:${curLeft}px; background-color: '';
         width:${trial.mask_size}px; 
         height:${trial.mask_size}px"></div>`;
        ...
        var showStimuli = function(ts, callback) {
            if ( trial.display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    SetItem('item'+pos[i],trial,trial.arrowArray[0][i], trial,trial.arrow_rotation[0][i]);
        ...
        var showMask = function(ms, callback) {
            if ( trial.mask_display_time > 0 ) {
                ...
                //    SetColor('item'+pos[i], item_mask_colors[i]); 
                    //  SetItem('item'+pos[i]);
                    // SetItem('item2'+pos[i],trial,trial.maskArray, trial);
            

                    // SetItem('item2'+pos[i],trial,trial.maskArray, trial);
                    SetItem('item2'+pos[i],trial,trial.maskArray[0][i][0], trial,trial.mask_rotation[0][i]);
     ...
     -------------------------------- */ 
        ...
        function draw(param) {
           ...
           /* var positionDeg = 157;
            ...
            html += `<div class='contMemoryChoice'
            id='backgroundRing' style='position:absolute;
        rgb(200, 5, 5); top: ${topPx}px; left: ${leftPx}px;'></div>`;   */
        ...
        */
          ...
          // document.getElementById('reportDiv').src = handleCM

          // var image = document.getElementsByClassName("contMemoryBox");
          //  image.src = handleCM



            html =  '<img src="' + gaborgen(360 - sceneNum, 10) + '"/>' + '</div>';
           'style= "background-image: url("img/mybackground.png"); position: absolute;"></div>';
           
            html += `<div id='backgroundRing2' 
       ...
       left: -118px; 
           ...
           ///THIS WORKS:
            //html += '<img src="' + handleCM + '"/>' + '</div>';
            //'style= "position: absolute; "></div>';


            
              
              //  '<div id="' + 'backgroundRing' + '"><img src="' + handleCM + ' "' +
              // 'style="width:30%; transform: rotate(' + (180) + 'deg)"></div>';
           //   'style="position: absolute; rgb(200, 5, 5); width:' + width +'px;  height: ' + height + 
           //   'px; transform: rotate(' + (param + wsp) + 'deg)"></div>';
            //   'style="width: "' + wheel_radius*2 + '"px;transform: rotate(' + (param + wsp) + 'deg)"></div>';
         //   display_element.innerHTML = html;
            ...
            //document.getElementById('backgroundRing').innerHTML = html;
            document.getElementById('reportDiv').innerHTML = html;
            
            //document.getElementById('contMemoryFixation')
        ...
        function displayWheel(){
           ...
           /* var html = `<div id='backgroundRing' 
            ...
            var html = `<div id='backgroundRing2' 
        style='background-image: url("img/mybackground.png");
            position: absolute;
...
*/
            ...
            background-color: rgb(205, 206, 206); top: ${topPx}px; left: ${leftPx}px;'></div>`;
                ...
                html += `<div class='contMemoryChoice' colorClicked='${deg}' 
                id='colorRing${deg}' style='position:absolute;
            rgb(205, 206, 206); top: ${topPx}px; left: ${leftPx}px;'></div>`;
            }
           ...
           /* function draw(param) {
            ...
            '<img src="' + gaborgen(360 - sceneNum, 10) + '"/>' +
                ...
                '<div id="' + 'reportDiv' + '"><img src="' + handle + '"' +
              // 'style="width:30%; transform: rotate(' + (180) + 'deg)"></div>';
       ...
       // document.addEventListener('mousemove', updateAngle);
        ...
        var getResponse = function() {
            ...
            if (trial.which_test==-1) {
                ...
                SetItem('item2'+pos[0],trial,trial.respCirc, trial,0);
