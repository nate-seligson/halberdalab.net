...
jsPsych.plugins['continuous-colorCM12'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-colorCM12',
        description: '',
        parameters: {
            set_size: {
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        //const xyCenter = findPos(reportDiv);
...
//this works ok at getting them vertically aligned
      //   let curTop = (Math.cos((Math.PI*2)/(trial.num_placeholders)*6)*(trial.radius)) 
      //   - trial.mask_size/2 + (center) + (i *10);
    // let curLeft = (Math.sin((Math.PI*2)/(trial.num_placeholders)*6)*(trial.radius)) 
      //   - trial.mask_size/2;


//assume we have a 4x4 grid? 

if (i < 4) {

var possibleTopLocs = [center, (center - 100), (center - 200), (center + 100), (center + 200)];
var possinleLeftLocs = [(center - 48), (center - 48), (center - 48), (center - 48),(center - 48)];
curTop = possibleTopLocs[i];
curLeft = possinleLeftLocs[1];

} else  {

    var possibleTopLocs = [center, (center - 100), (center - 200), (center + 100), (center + 200)];
    var possinleLeftLocs = [(center - 48), (center - 248), (center - 48), (center - 48),(center - 48)];
    curTop = possibleTopLocs[i - 4];
    curLeft = possinleLeftLocs[2];
}
        
      //let curTop =center - (i *100);
  

            html += `<div id="item${i}" class="contMemoryItem" draggable = "true"
       ...
       // let curLeft = (Math.sin((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius2[i]))
         //   - trial.mask_size/2 + center;
            ...
            let curTop = (Math.cos((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius2[0]))
                - trial.mask_size/2 + center;
            let curLeft = (Math.sin((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius2[0]))
                - trial.mask_size/2 + center;
