 ...
 * Tim Brady Dec. 2020
 ...
 jsPsych.plugins['continuous-colorCM2'] = (function() {
...
/////////CM ADDED
       item_maskcolors: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Actual MASK colors to show (optional)',
        default: [],
                array: true,
                description: 'If not empty, should be a list of colors to show, in degrees of color wheel, of same length as set_size. If empty, random colors with min distance between them of min_difference [option below] will be chosen. '
      },
...
/////////CM ADDED
       maskTime: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Duration of mask presentation',
        default: [],
                array: true,
                description: 'If not empty, should be the duration of mask, in ms. '
      },
       ...
       mask_display_time: {
        ...
        default: 500,
        description: 'Time in ms. to display mask'				
            },
   ...
   plugin.trial = function(display_element, trial) {
     ...
     var html = `
         ...
         var startTrial = function() {
             ...
             jsPsych.pluginAPI.setTimeout(()=>showMask(performance.now()), trial.delay_start);
         };
         if (trial.click_to_start) {
...
/* Show the MASK (CM):  
         ...
         var item_maskcolors = trial.item_maskcolors;
         if (trial.item_maskcolors.length==0) {
             item_maskcolors = GetColorsForTrial(trial.set_size, trial.min_difference);
         }
         var start_request_anim;
     var last_frame_time;
     var showMask = function(ms) {
             if ( trial.mask_display_time > 0 ) {
                 for (var i=0; i<trial.set_size; i++) {
                      SetColor('item'+pos[i], item_maskcolors[i]);  
                 }
             }
       start_request_anim = ms;
       last_frame_time = ms;
       showMask(ms);
     };
     ...
     var hideMask = function(ms) {
       var last_frame_duration = ms - last_frame_time;
       last_frame_time = ms;
       if (ms - start_request_anim 
         >= trial.mask_display_time - (last_frame_duration/2)) { 
         actual_stim_duration = ms - start_request_anim;
         for (var i=0; i<trial.set_size; i++) {
           document.getElementById('item'+pos[i]).style.backgroundColor = '';
         }        
         requestAnimationFrame(delayUntilProbe);
