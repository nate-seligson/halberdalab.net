...
jsPsych.plugins['color-precision2'] = (function() {
    var plugin = {};
    plugin.info = {
        name: 'color-precision2',
        description: '',
        parameters: {
            set_size: {
        ...
        var showStimuli = function(ts, callback) {
            if ( trial.display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    SetColor('maskItem'+pos[i], item_colors[i]);  
        ...
        var showMask = function(ms, callback) {
            if ( trial.mask_display_time > 0 ) {
                ...
                //    SetColor('item'+pos[i], item_mask_colors[i]); 
                    //  SetItem('item'+pos[i]);
                    SetItem('maskItem'+pos[i],trial,trial.maskArray[0][i][0], trial);
