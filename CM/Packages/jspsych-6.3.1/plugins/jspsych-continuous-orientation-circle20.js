/**
 * jspsych-continuous-orientation
 * plugin for continuous orientation report, using wheel similar to that by Schurgin et al.
 * 
...
//Last updated by Caroline MYERS (Johns Hopkins)- [11/12/21]

  */

var continuous_color_css_added = false;

jsPsych.plugins['continuous-orientation-circle20'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-orientation-circle20',
        description: '',
        parameters: {
            set_size: {
            ...
            gaborMasks: {
                ...
                default: [],
                array: true,
                description: 'The array of gabors at each location that serve as a mask. '
            },
            ...
            numMasks: {
                type: jsPsych.plugins.parameterType.INT,
                pretty_name: 'number of gabors to be drawn at each location as masks',
                //default: 200, //200
                default: 5,
                description: 'number of gabors to be drawn at each location as masks.'
            },
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        const numMasks = trial.numMasks;
        var gaborMasks = trial.gaborMasks; 
        //${trial.bg_color}
        if (trial.item_colors.length==0) {
            item_colors = GetColorsForTrial(trial.set_size, trial.min_difference);
        }
 
        var arrow_rotation = trial.arrow_rotation;
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            html += `<div id="item${i}" class="contMemoryItem" 
                 style="top:${curTop}px; transform: rotate(${0}deg); left:${curLeft}px; border-radius: 50%; background-color: '';
...
/*
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            html += `<div id="maskitem${i}gab${j}" class="contMemoryItem" 
                 style="top:${curTop + getRandomIntInclusive(-45,45)}px; transform: rotate(${trial.mask_rotation[0][i]}deg); left:${curLeft + getRandomIntInclusive(-45,45)}px; 
...
//this is the one we've been using
        ...
        var width = trial.radius * 2 + trial.mask_size * 2 + 50;
        var height = trial.radius * 2 + trial.mask_size * 2 + 50;
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            for (var j=0; j < numMasks; j++) {
            let curTopMask = (Math.cos((Math.PI*2)/(trial.num_placeholders)*i)*trial.radius)
                 - trial.mask_size/2 + center;
            let curLeftMask = (Math.sin((Math.PI*2)/(trial.num_placeholders)*i)*trial.radius)
                 - trial.mask_size/2 + center;
                 
        
            html += `<div id="maskitem${i}gab${j}" class="contMemoryItem" 
                 style="top:${curTopMask}px; transform: rotate(${trial.mask_rotation[0][i]}deg); left:${curLeftMask}px; 
                 backgroundSize = 'contain'; 
            
         width:${trial.mask_size}px; 
         height:${trial.mask_size}px; background-color: '#DDDDDD'; background-image: url("img/mask2.png") "></div>`;
        ...
        */
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            html += `<div id="maskitem${i}gab${j}" class="contMemoryItem" 
                 style="top:${curTop + getRandomIntInclusive(-20,20)}px; left:${curLeft + getRandomIntInclusive(-20,20)}px; background-color: '';
         width:${trial.item_size}px; 
         height:${trial.item_size}px"></div>`;
        ...
        html += `<span id="contMemoryFixation" style="cursor: pointer">+</span>
         ...
         width:${trial.item_size}px; 
         height:${trial.item_size}px; background-color: '#DDDDDD'; background-image: url("img/mybackground.png") "></div>
        ...
        // document.display_element.style.backgroundImage = `url('img/mybackground.png')`;
        
        // display_element.querySelector('#contMemoryStartTrial').removeEventListener('click', startTrial);
        //displayWheel();
        ...
        var showStimuli = function(ts, callback) {
            if ( trial.display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    SetGabor('item'+pos[i],trial,trial.arrowArray[0][i], trial,trial.arrow_rotation[0][i]);
                    //  SetItem('item'+pos[i],trial,trial.arrowArray[0][i], trial,270);
     ...
     -------------------------------- */ 
        ...
        var showMask = function(ms, callback) {
            if ( trial.mask_display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    for (var j=0; j < numMasks; j++) {
                    //SetItem('maskitem'+pos[i],trial,trial.maskArray[0][i][0], trial,trial.mask_rotation[0][i]);

                    SetMask('maskitem'+pos[i]+'gab'+[j],trial,trial.maskArray[0][i], trial,gaborMasks[0][0][j]);
                    //SetMask('maskitem'+pos[i]+'gab'+[j],trial,trial.maskArray[0][i], trial,generateColorsMaxMin(numMasks,10, 20, 359)[j]);

                    //SetMask('maskitem'+pos[i]+'gab'+[j],trial,trial.maskArray[0][i], trial,trial.mask_rotation[0][i]);


                    //SetMask2('maskitem'+pos[i],trial,trial.maskArray[0][i], trial,trial.mask_rotation[0][i]);
                    //SetMask('maskitem'+pos[i],trial,trial.maskArray[0][i], trial,trial.mask_rotation[0][i]);
                    //SetMask('maskitem'+pos[i],trial,trial.maskArray[0][i], trial,trial.mask_rotation[0][i]);
 ...
 -------------------------------- */ 
     ...
     >= trial.mask_display_time - (last_mask_frame_duration/2)) { 
                ...
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    //cm added:
                    document.getElementById('maskitem'+pos[i]+'gab' + [j]).style.backgroundImage = '';
                    }
                } 
       ...
       // var param = 0;
        ...
        function draw(param) {
            ...
            if (param == 360) {
                ...
                var handleCM = 'img/mybackground.png';
            } else {
...
//THIS IS WHERE WE CHANGE WHAT APPEARS DURING THE RESPONSE
            html =  '<img src="' + gaborgen(360 - sceneNum, 100) + 
            '" style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
            'px; border-radius: 50%; width:' + 100 + 'px; height: ' + 100 + 'px;"/>' + '</div>';
           'style= "position: absolute;"></div>';
           
            html += `<span id="fixCross" style="cursor: pointer"></span>
            <div id='backgroundRing2' 
           style='
               position: absolute;
       
       bgSize = ${wheel_radius*3};
       ...
       left: -118px; 
               ...
               width: ${wheel_radius*3}px;
               height: ${wheel_radius*3}px'>&nbsp;
       </div>`;
    ...
    document.getElementById('reportDiv').innerHTML = html;
        ...
        function displayWheel(){
            ...
            html += `' style='background-image: url("img/mybackground.png");
                position: absolute;
        ...
        left: -118px; 
           ...
           // THIS IS WHERE WE ADD OUR FIX + WHILE GABORS AND MASKS ARE PRESENT
            ...
            style='background-image: url("img/mybackground.png");
                position: absolute;
       ...
       document.getElementById('contMemoryFixation').innerHTML = html;
        ...
        var getResponse = function() {
            ...
            if (trial.which_test==-1) {
               ...
               //THIS IS WHERE WE WILL PUT OUR RESPONSE
               // SetItem('maskitem'+pos[0],trial,trial.respCirc, trial,0);
        ...
        var judge_response = function(e){ 
            ...
            if (trial.which_test==-1) {
                ...
                var err = Math.round(wrap(response_angle - wheel_spin))
                      - arrow_rotation[trial.which_test];
                if (trial.color_wheel_list_options.length>0 
                         || trial.color_wheel_num_options<360) {
                    err = colClicked - arrow_rotation[trial.which_test];
                }
                if (err>180) { err-=360; }
                if (err<=-180) { err+=360; }
            }
 ...
 var mouseclickevent = function() {
                ...
                var trial_data = {
                   // "fix_duration": start_time - fixstart,
                    ...
                    "startingAngle": trial.starting_value,

                    'actual_mask_duration': actual_mask_duration,
                    'actual_stim_duration': actual_stim_duration,
                    'arrow_orientation': arrow_rotation[0][0],
                    'orientation_of_target': arrow_rotation[0][0],
                    'allOris': arrow_rotation,
    ...
    function SetItem(id, deg, myImg, trial,rotdegrees) {
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
        var amt = rotdegrees;
    ...
    function SetGaborOld(id, deg, myImg, trial,rotdegrees) {
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
        var amt = rotdegrees;
        var myImg = gaborgen(360 - deg, 10) 
        document.getElementById(id).style.backgroundImage = `url('${myImg.toString()}')`;
    }


    function SetGabor(id, deg, myImg, trial,rotdegrees) {
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
        var amt = rotdegrees;
      ...
      var width = trial.radius * 2 + trial.item_size * 2 + 50;
       ...
       document.getElementById(id).style.backgroundSize = 'contain';
        var myImg = gaborgen(rotdegrees, 100) 
        document.getElementById(id).style.backgroundImage = `url('${myImg.toString()}')`;
    ...
    function SetMaskOld(id, deg, myImg, trial,rotdegrees) {
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
        var amt = rotdegrees;
        var width = 100;  //var width = trial.radius * 2 + trial.item_size * 2 + 50;
        var height = 100; //var height = trial.radius * 2 + trial.item_size * 2 + 50;
       ...
       - trial.item_size/2 + center;
        ...
        var myImg = gaborgen(rotdegrees, 20) //UNCOMMENT FOR REAL ONE
        var elem = document.createElement("img");
        elem.src = gaborgen(rotdegrees, 20)

        document.getElementById(id).appendChild(elem);
        var box23 = document.getElementById(id);
        $('#box23').html('<img src="' +  + '"/>');
       


        var box23 = document.getElementById(id);
        box23.style.background= `url('${myImg.toString()}')left 90px no-repeat,` + 
        "url(" + 'img/mask3.png' + ") center left 48px no-repeat, \
        url('img/mask.png') center right 90px no-repeat, \
        url('img/mask2.png') center right 90px no-repeat";
        box23.style.backgroundSize= "90px, 90px,90px,90px";//"90px"
        box23.style.borderRadius =  "90px, 90px,90px,90px"
        box23.style.backgroundSize= "90px, 90px,90px,90px";//"90px"

       
       // document.getElementById(id).appendChild(elem);
       // elem.src = 'img/mask2.png';
       document.getElementById(id).innerHTML ='<img src="' + gaborgen(2, 5) + '"/> + " style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
            'px; border-radius: 50%; width:' + 100 + 'px; height: ' + 100 + 'px;"/>' + '</div>';
           'style= "position: absolute;"></div>';


           
           document.getElementById(id).innerHTML ='<img src="' + `url('${myImg.toString()}')` + '"/>';


           document.getElementById(id).innerHTML ='<img src="' + gaborgen(2, 4) + '"/>';
          '"></div>';


        document.getElementById(id).src = gaborgen(2, 3) + 
            '" style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
            'px; border-radius: 50%; width:' + 100 + 'px; height: ' + 100 + 'px;"/>' + '</div>';
           '></div>';
...
//////FIRST ONE
        ...
        document.getElementById(id).style.backgroundSize =  "90px, 90px";//"90px"
        document.getElementById(id).style.backgroundPosition = "right bottom";
        document.getElementById(id).style.backgroundImage += `url('${myImg.toString()}')`; //UNCOMENT FOR REAL ONE
       document.getElementById(id).style.backgroundPosition = "right top";
        document.getElementById(id).style.backgroundImage += `url('${myImg2.toString()}')`;
        document.getElementById(id).style.borderRadius = 80;//80; 

       // let curr = document.getElementById(id);
       // <curr style="height: 100px; width: 100px; border: 1px solid black; background-image: url('${myImg2.toString()}');"></curr>;
    ...
    function SetMask(id, deg, myImg, trial,rotdegrees) {
       ...
       - trial.item_size/2 + center;
       document.getElementById(id).style.backgroundSize = 'contain';
        var myImg = gaborgen(rotdegrees, 100) //UNCOMMENT FOR REAL ONE


   //THIS WORKS
      //  document.getElementById(id).innerHTML +=   '<img src="' + gaborgen(rotdegrees, 100) + 
     //   '" style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
       // 'px; border-radius: 50%; width:' + trial.item_size + 'px; height: ' + trial.item_size + 'px;"/>' + '</div>';


        document.getElementById(id).innerHTML +=   '<img src="' + gaborgen(rotdegrees, 100) + 
        '" style = "top:' + (curTopCenter) + '; left:' + (curLeftCenter) + 
        '; border-radius: 50%; width:' + trial.item_size + 'px; height: ' + trial.item_size + 'px; "/>' + '</div>';


//save image
window.open('', document.getElementById(id).toDataURL());

    }



    function SetMaskNewestCurrent(id, deg, myImg, trial,rotdegrees) {
        var myImg = gaborgen(rotdegrees, 100) //UNCOMMENT FOR REAL ONE
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
        document.getElementById(id).style.backgroundImage = `url('${myImg.toString()}')`;
    ...
    function SetMaskWORKING(id, deg, myImg, trial,rotdegrees) {
       ...
       //document.getElementById(id).style.backgroundSize = 'contain';
        ...
        html =  '<img src="' + gaborgen(2, 5) + 
        '" style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
        'px; border-radius: 50%; width:' + trial.item_size + 'px; height: ' + trial.item_size + 'px;"/>' + '</div>';
       'style= "position: absolute;"></div>';
       
        html += `<span id="thisdiv" style="cursor: pointer"></span>
        <div id='thisdiv' 
       style='
           position: absolute;
