...
jsPsych.plugins['continuous-line5'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-line5',
        description: '',
        parameters: {
            set_size: {
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        var width = trial.radius * 2 + trial.item_size * 2 + 62;
        var height = trial.radius * 2 + trial.item_size * 2 + 62;
        ...
        var showStimuli = function(ts, callback) {
            if ( trial.display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    SetGabor('item'+pos[i],trial,trial.arrowArray[0][i], trial,trial.line_length[0][i],trial.line_orientation[0][i]);
...
'px; width:' + (centerLineLength) + 'px; height: ' + 10 + 'px;"/>' + '</div>';
'style= "position: center center;"></div>';
    ...
    document.getElementById('reportDiv').innerHTML = html;
        ...
        function displayWheel(){
            ...
            var wheel_radius = trial.radius + trial.item_size + 5;//trial.radius + trial.item_size + 5;
       ...
       left:  ${curLeftCenter}px;
           ...
           // THIS IS WHERE WE ADD OUR FIX + WHILE GABORS AND MASKS ARE PRESENT
            ...
            style='border: border-radius: 50%; background-image: url("img/noBar_wheelCM.png");
                position: absolute;
        
        bgSize = ${wheel_radius*2};


        top: ${curTopCenter-130}px; 
        left: ${curLeftCenter-130}px; 
        ...
        var updateAngle = function(e) {
          ...
          //  var curAngle = Math.atan2(relY,relX);
           // response_angle = curAngle / Math.PI * 180.0;
            ...
            // Normalize y coordinate to range from -1 to 1
          //  let normalized = (realY - MIDPOINT) / (200 - MIN_WIDTH);
          let normalized = (e.clientY - MIN_Y) / Y_RANGE;
          
            // Clamp value to range between -1 and 1
           // normalized = Math.min(Math.max(normalized, -1), 1);
           normalized = Math.min(Math.max(normalized, -1), 1);
           console.log(normalized);
            // Map normalized value to range between MIN_WIDTH and MAX_WIDTH
            let width = CENTER_WIDTH + (normalized) * RANGE / 2;
 ...
 */
 ...
 var mouseclickevent = function() {
    ...
    function SetGabor(id, deg, myImg, trial,length,line_ori) {
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
