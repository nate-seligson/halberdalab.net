 ...
 * Tim Brady Dec. 2020
 * 
 * Adapted by Caroline Myers for 2 systems experiment. Now takes additional arguments, including text 
 * to be displayed before each trial, and ensembleCond (1 or 2). 1 corresponds to 1 item highlighted, 
 * 2 corresponds to all items being highlighted (aka report the mean.)
  */

var continuous_color_css_added = false;

jsPsych.plugins['continuous-color-ensemble'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-color-ensemble',
        description: '',
        parameters: {
            set_size: {
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        var min_difference = trial.min_difference;
        var maskNum = trial.maskNum;
        //${trial.bg_color}
        if (trial.item_colors.length==0) {
            item_colors = GetColorsForTrial(trial.set_size, trial.max_difference,min_difference);
        }
        ...
        if (trial.ITIText ==1) {
            trial.startText = 'Pay close attention to the colors of each dot. \n Click the + to start this trial.';
        }

        if (trial.ITIText ==2) {
            trial.startText = 'Pay close attention to the colors of every dot. \n Click the + to start this trial.';
        }

        if (trial.ITIText ==3) {
            trial.startText = 'Pay close attention to the colors of the dots. \n Click the + to start this trial. Click the + to start this trial.';
        }

        if (trial.ITIText ==0) {
            trial.startText = 'Click the + to start this trial.';
        }


        //var startText = 'Click the + to start this trial.';
        if (!trial.click_to_start) {	trial.startText;	}
        var html = `
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            ...
            html += `<div id="item${i}" class="contMemoryItem" 
                 style="top:${curTop}px; left:${curLeft}px; background-color:${trial.bg_color};
     ...
     -------------------------------- */ 
        ...
        if (trial.item_mask_colors.length==0) {
            item_mask_colors = GetColorsForTrial(trial.set_size, trial.max_difference,trial.min_difference);
        }
        min_difference = trial.min_difference;
 ...
 -------------------------------- */ 
     ...
     -------------------------------- */ 
        ...
        var getResponse = function() {
            ...
            if (trial.ensembleCondition==1) {
                for (var i=0; i<trial.set_size; i++) {
                    //    SetColor('item'+pos[i], item_mask_colors[i]); 
                    //  SetItem('item'+pos[i]);
                    //SetItem('item2'+pos[i],trial,trial.maskArray[0][i][0], trial);
                    document.getElementById('item' + pos[i]).style.border = '3px dashed gray';
            ...
            if (trial.ensembleCondition==2) {

                for (var i=0; i<trial.set_size; i++) {
                    //    SetColor('item'+pos[i], item_mask_colors[i]); 
                    //  SetItem('item'+pos[i]);
                    //SetItem('item2'+pos[i],trial,trial.maskArray[0][i][0], trial);
                    document.getElementById('item' + pos[i]).style.border = '3px solid black';
    ...
    function GetColorsForTrial(setSize, max_difference, min_difference) {
        var items = [];
        ...
        for (var j=1; j<=setSize-1; j++) {
            ...
            for (var c=0;c<360; c++) { 
                isValid = !tooClose(whichCol,c, max_difference, min_difference);
                for (var testAgainst=0;testAgainst<j;testAgainst++) {
                    if (isValid && tooClose(items[testAgainst],c,max_difference,min_difference)) {
                        isValid = false;
                    }
                }
                if (isValid) {
    ...
    ///// max_difference MUST > min diff * setSize !!!!!!//////
 
    function tooClose(startcol,endcol,max_difference,min_difference) {
         if (isNaN(startcol) || isNaN(endcol)) {
        //     console.log('uh oh');
            return false;
        }

        if (typeof startcol == 'undefined' || typeof endcol == 'undefined') {
            return true;
        }
        if (Math.abs(startcol-endcol)>=max_difference) {
            return true;
        }

        if (Math.abs(startcol-endcol)<=min_difference) {
            return true;
        }
        ...
        if (Math.abs(startcol+360 - endcol)<=min_difference) {
            return true;
        }	
