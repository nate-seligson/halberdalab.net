...
jsPsych.plugins['ori-report-last-question'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'ori-report-last-question',
        description: '',
        parameters: {
            set_size: {
        ...
        var judge_response = function(e){ 
            ...
            if (trial.which_test==-1) {
                ...
                var err = Math.round(wrap(response_angle - wheel_spin))
                      - gabor_orientation[trial.which_test];
                if (trial.color_wheel_list_options.length>0 
                         || trial.color_wheel_num_options<360) {
                    err = colClicked - gabor_orientation[trial.which_test];
                }
                if (err>180) { err-=360; }
                if (err<=-180) { err+=360; }
            }
