...
jsPsych.plugins['continuous-line7'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-line7',
        description: '',
        parameters: {
            set_size: {
            ...
            all_possible_lengths: {
                type: jsPsych.plugins.parameterType.INT,
                pretty_name: 'Should be 360 values.',
                default: [],
                array: true,
                description: 'If not empty, should be a list of colors to show, in degrees of color wheel, of same length as set_size. If empty, random colors with min distance between them of min_difference [option below] will be chosen. '
            },
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
         width: ${(trial.radius*2) + (trial.item_size*2)};
         height: ${(trial.radius*2) + (trial.item_size*2)};   
         display: flex;
         margin: 0 auto;
         align-items: center;
         justify-content: center;  
        ...
        var line_length = trial.line_length;
        var line_orientation = trial.line_orientation;
        var all_possible_lengths = trial.all_possible_lengths;
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
             ////HERE WE SUBTRACT 20 FROM THE RADIUS TO MAKE OUR STIMULI APPEAR CLOSER TO CROSS. 
            let curTop = (Math.cos((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius-20))
                 - trial.real_item_size/2 + center;
                 ////HERE WE SUBTRACT 20 FROM THE RADIUS TO MAKE OUR STIMULI APPEAR CLOSER TO CROSS. 
            let curLeft = (Math.sin((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius-20))
                 - trial.real_item_size/2 + center;
         ...
         width:${trial.real_item_size}px; 
         height:${trial.real_item_size}px"></div>`;
        ...
        for (var i=0; i < trial.num_placeholders; i++) {
            let curTop = (Math.cos((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius-40))
                 - trial.mask_size/2 + center;
            let curLeft = (Math.sin((Math.PI*2)/(trial.num_placeholders)*i)*(trial.radius-40))
                 - trial.mask_size/2 + center;
                 

        
            html += `<div id="maskItem${i}" class="contMemoryItem" 
                 style="top:${curTop}px; transform: rotate(${trial.mask_rotation[0][i]}deg); left:${curLeft}px; 
                 background-image: 'img/mybackground.png';
        ...
        var showStimuli = function(ts, callback) {
            if ( trial.display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    SetItem('item'+pos[i],trial,trial.arrowArray[0][i], trial,trial.line_length[0][i],trial.line_orientation[0][i]);
                    //  SetMask('item'+pos[i],trial,trial.arrowArray[0][i], trial,270);
        ...
        var showMask = function(ms, callback) {
            if ( trial.mask_display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    SetMask('maskItem'+pos[i],trial,trial.maskArray[0][i][0], trial,trial.mask_rotation[0][i]);
                }
            }
 ...
 -------------------------------- */ 
     ...
     >= trial.mask_display_time - (last_mask_frame_duration/2)) { 
                ...
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    //cm added:
                    document.getElementById('maskItem'+pos[i]).style.backgroundImage = '';
                }        
                requestAnimationFrame((ts)=>{
                    maskCallback(ts);
                    // delayUntilProbe(ts);
     ...
     -------------------------------- */ 
       ...
       // var param = 0;
        ...
        function draw(param) {
            ...
            currentDegAngle = ("" + param);		
            //currentDegAngle = ("" + param).slice(-6);
...
/*
This is where things get tricky. We have 360 degrees around the circle, 
so we basically need to use that as an index to get what line length it should actually be. 
lengthOfInnerLine = array of all possible line lengths[currentDegAngle]

*/
centerLineLength = currentDegAngle;
 //centerLineLength = all_possible_lengths[currentDegAngle];
...
//html =  currentDegAngle.toString() +'<img src="' + gaborgen(360 - currentDegAngle, trial.sf) + 
html = '<img src="img/myline'  + '.jpg' +
' " style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
'px; width:' + (10) + 'px; height: ' + centerLineLength + 'px;"/>' + '</div>';
'style= "position: center center;"></div>';
    ...
    document.getElementById('reportDiv').innerHTML = html;
        ...
        function displayWheel(){
            ...
            var wheel_radius = trial.radius + trial.item_size;//trial.radius + trial.item_size + 5;
       ...
       left:  ${curLeftCenter}px;
            ...
            let curTop = ((Math.cos((Math.PI*2)/(trial.num_placeholders)*1)*trial.radius)
            -  center) - ((trial.radius/2) + (trial.item_size/2))+10;
            let curLeft = ((Math.sin((Math.PI*2)/(trial.num_placeholders)*1)*trial.radius)
            -  center)-((trial.radius/2)  + (trial.item_size/2))+10;
        
            let someOtherVarTop = 0;
            let someOtherVarLeft= 0;
        ...
        top: ${someOtherVarTop}px; 
        left: ${someOtherVarLeft}px; 
...
const maxDistFromMidpoint = midpoint - 100; //so that people don't go all the way to the edge
const minWidth = 20;
const maxWidth = 200;

const distanceFromMidpoint = midpoint - e.clientY;
const heightPercent = distanceFromMidpoint / maxDistFromMidpoint;

const proportionalValue = (heightPercent + 1) / 2;
const param1 = minWidth + proportionalValue * (maxWidth - minWidth);

const param = Math.round(Math.max(minWidth, Math.min(maxWidth, param1)) * 2)/2;
        ...
        var getResponse = function() {
            ...
            if (trial.which_test==-1) {
                ...
                SetMask('maskItem'+pos[0],0,trial.respCirc, trial,0);
        ...
        var judge_response = function(e){ 
            ...
            if (trial.which_test==-1) {
                ...
                var err = Math.round(wrap(response_angle - wheel_spin))
                      - line_length[trial.which_test];
                if (trial.color_wheel_list_options.length>0 
                         || trial.color_wheel_num_options<360) {
                    err = colClicked - line_length[trial.which_test];
                }
                if (err>180) { err-=360; }
                if (err<=-180) { err+=360; }
            }
 ...
 */
 ...
 var mouseclickevent = function() {
                ...
                var final_actual_length = all_possible_lengths[param];
                

                if (final_angle > 179) {
                    responsetransformed = final_angle - 180;
                } else {
                    responsetransformed = final_angle;
                }
                ...
                var trial_data = {
                    "rt": response_time,
                    "responseAngleRaw": final_angle,
                    "final_actual_length": final_actual_length,
                    "startingLength": trial.starting_value,
                    'actual_mask_duration': actual_mask_duration,
                    'actual_stim_duration': actual_stim_duration,
                    'length_of_target': line_length[0][0],
                    'random_ori_of_target': line_orientation[0][0], 
                    'allLengths': line_length,
                    'allOris': line_orientation,
        ...
        var endTrial = function(trial_data){
            ...
            var trial_data = {
                ...
                'actual_mask_duration': actual_mask_duration,
                'actual_stim_duration': actual_stim_duration,
                'arrow_orientation': line_length[0][0],
                'orientation_of_target': line_length[0][0],
                'allOris': line_length,
                //"rtNEW": response_time,
    ...
    /* Helper functions
      ...
      function SetPostCue(id, deg, myImg, trial,rotdegrees) {
        ...
        document.getElementById(id).style.backgroundSize = trial.item_size + 'px' + trial.item_size + 'px';
    }
    ...
    function SetItem(id, deg, myImg, trial,length,line_ori) {
        deg=(deg>=360) ? deg-360:deg;
        deg=(deg<0) ? deg+360:deg;
        ...
        let curTopCenter = (Math.cos((Math.PI*2)/(trial.num_placeholders)*1)*trial.radius) - trial.item_size/2 + center;
        let curLeftCenter = (Math.sin((Math.PI*2)/(trial.num_placeholders)*1)*trial.radius) - trial.item_size/2 + center;
        

        temp = 'img/myline'
        document.getElementById(id).style.backgroundImage = `url('${temp.toString()}.jpg')`;
       // document.getElementById(id).style.backgroundPosition = "absolute";
        document.getElementById(id).style.backgroundPosition = "center center";
        document.getElementById(id).style.backgroundSize = length + 'px 10px';

         

        document.getElementById(id).style.backgroundRepeat = "no-repeat";
        document.getElementById(id).style.transform = 'rotate(' + line_ori + 'deg)';
       // document.getElementById(id).style.backgroundOrigin = "content-box";
...
- trial.item_size/2 + center;
       ...
       var center = width/2;
       // var myImg = '<img src="' + gaborgen(360 - deg, 10) + '"/>'
       let curTopCenter = (Math.cos((Math.PI*2)/(trial.num_placeholders)*1)*trial.radius)
       - trial.item_size/2 + center;
       let curLeftCenter = (Math.sin((Math.PI*2)/(trial.num_placeholders)*1)*trial.radius)
       - trial.item_size/2 + center;
      ...
      //  document.getElementById(id).style.backgroundImage = gaborgen(360 - deg, 10)
