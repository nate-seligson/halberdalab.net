...
jsPsych.plugins['continuous-colorCM5'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-colorCM5',
        description: '',
        parameters: {
            set_size: {
            ...
            ensembleCondition: {
                type: jsPsych.plugins.parameterType.INT,
                pretty_name: 'for ensemble exp, report item or ensemble',
                default: 1,
                description: 'for ensemble exp, report item or ensemble'
            },	
            ...
            mask_radius: {
                ...
                default: 80, //80
                description: 'Diameter of each circle in pixels, unused.'
            },
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        if (trial.item_colors.length==0) {
            item_colors = GetColorsForTrial(trial.set_size, trial.max_difference);
        }
 ...
 -------------------------------- */ 
    ...
    function GetColorsForTrial(setSize, max_difference) {
        var items = [];
        ...
        for (var j=1; j<=setSize-1; j++) {
            ...
            for (var c=0;c<360; c++) { 
                isValid = !tooClose(whichCol,c, max_difference);
                for (var testAgainst=0;testAgainst<j;testAgainst++) {
                    if (isValid && tooClose(items[testAgainst],c,max_difference)) {
                        isValid = false;
                    }
                }
                if (isValid) {
 ...
 ///// max_difference MUST > min diff * setSize !!!!!!//////
    ...
    function tooClose(startcol,endcol,max_difference) {
        ...
        if (Math.abs(startcol-endcol)<=10) {
            return true;
        }
