...
jsPsych.plugins['continuous-face4'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'continuous-face4',
        description: '',
        parameters: {
            set_size: {
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        var gabor_rotation = trial.gabor_rotation;
        ...
        var showStimuli = function(ts, callback) {
            if ( trial.display_time > 0 ) {
                for (var i=0; i<trial.set_size; i++) {
                    ...
                    SetGabor('item'+pos[i],trial,trial.arrowArray[0][i], trial,trial.gabor_rotation[0][i]);
...
html = '<img src="img/export' + (sceneNum) + '.png' +
' " style = "top:' + (curTopCenter) + 'px; left:' + (curLeftCenter) + 
'px; border-radius: 50%; width:' + 90 + 'px; height: ' + 90 + 'px;"/>' + '</div>';
        ...
        var judge_response = function(e){ 
            ...
            if (trial.which_test==-1) {
                ...
                var err = Math.round(wrap(response_angle - wheel_spin))
                      - gabor_rotation[trial.which_test];
                if (trial.color_wheel_list_options.length>0 
                         || trial.color_wheel_num_options<360) {
                    err = colClicked - gabor_rotation[trial.which_test];
                }
                if (err>180) { err-=360; }
                if (err<=-180) { err+=360; }
            }
 ...
 var mouseclickevent = function() {
                ...
                var trial_data = {
                   // "fix_duration": start_time - fixstart,
                    ...
                    'gabor_orientation': gabor_rotation[0][0],
                    'orientation_of_target': gabor_rotation[0][0],
                    'allOris': gabor_rotation,
        ...
        var endTrial = function(trial_data){
            ...
            var trial_data = {
                ...
                'actual_mask_duration': actual_mask_duration,
                'actual_stim_duration': actual_stim_duration,
                'arrow_orientation': gabor_rotation[0][0],
                'orientation_of_target': gabor_rotation[0][0],
                'allOris': gabor_rotation,
                //"rtNEW": response_time,
    ...
    function SetGabor(id, deg, myImg, trial,rotdegrees) {
       ...
       //  var myImg = gaborgen(rotdegrees, trial.sf) 
        ...
        temp = 'img/export'
        document.getElementById(id).style.backgroundImage = `url('${temp.toString() + rotdegrees}.png')`;
       // document.getElementById(id).style.backgroundPosition = "absolute";
        ...
        document.getElementById(id).style.backgroundSize = "90px 90px";
