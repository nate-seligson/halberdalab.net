...
jsPsych.plugins['color-report-last-question'] = (function() {
 
    var plugin = {};

    plugin.info = {
        name: 'color-report-last-question',
        description: '',
        parameters: {
            set_size: {
    ...
    plugin.trial = function(display_element, trial) {
     ...
     --------------------------------*/
       ...
       #contMemoryBox {
        ...
        if (!trial.click_to_start) {	startText = 'Please select a color:';	}
        var html = `
        ...
        html += `<span id="contMemoryFixation" style="cursor: pointer"></span>
             <div id="contMemoryStartTrial" style="position: absolute; 
                 top:20px">${'Please select a color now:'}</div>
