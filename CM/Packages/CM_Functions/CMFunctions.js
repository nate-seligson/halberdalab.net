...
//****************************//
function GetLineLengthsForTrial(setSize, minDiff,startPoint,endPoint,myarray) {
    var items = [];
    var whichCol = myarray[Math.floor(Math.random()*myarray.length)];
    //var whichCol = getRandomIntInclusive(startPoint,endPoint); //359
    items.push(whichCol);
     
    for (var j=1; j<=setSize-1; j++) {
        var validLengths = new Array();
        for (var c=startPoint;c<(endPoint + 1); c++) { 
            isValid = !tooCloseOri(whichCol,c, minDiff);
            for (var testAgainst=0;testAgainst<j;testAgainst++) {
                if (isValid && tooCloseOri(items[testAgainst],c,minDiff)) {
                    isValid = false;
                }
            }
            if (isValid) {
        ...
        validLengths = jsPsych.randomization.shuffle(validLengths);
        items.push(validLengths[0]);
...
// get orientations that are min apart
function generateColorsMaxMin(setSizeInput,minDistance, maxRelDistance, MaxDistance) {
    var flagTest = 'bad'; // the current state of affairs 

    while (flagTest == 'bad') {
        Myarray = Array.from(Array(setSizeInput).keys());
        Myarray[0] = Math.round(getRandomArbitrary(0, 359));
...
function generateOrisMaxMin180(setSizeInput,minDistance, maxRelDistance, MaxDistance) {
    var flagTest = 'bad'; // the current state of affairs 

    while (flagTest == 'bad') {
        Myarray = Array.from(Array(setSizeInput).keys());
        Myarray[0] = Math.round(getRandomArbitrary(0, 179));
...
function GetOrisForTrial(setSize, minDiff) {
    var items = [];
    var whichCol = getRandomIntInclusive(0,359); //359
    items.push(whichCol);
...
function GetOrisForTrial360(setSize, minDiff) {
    var items = [];
    var whichCol = getRandomIntInclusive(0,359); //359
    items.push(whichCol);
...
function GetLengthsForTrial200(setSize, minDiff) {
    var items = [];
    var whichCol = getRandomIntInclusive(1,200); //359
    items.push(whichCol);
...
function GetOrisForTrial(setSize, minDiff) {
    var items = [];
    var whichCol = getRandomIntInclusive(0,179); //359
    items.push(whichCol);
...
function tooCloseOri(startcol,endcol,minDiff) {
