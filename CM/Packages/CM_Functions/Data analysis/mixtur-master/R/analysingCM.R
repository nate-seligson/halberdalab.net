...
get_summary_statistics <- function(data,
  ...
  if(unit == "degrees_180"){
    ...
    #CM CHANGED TO DIVIDE BY 2
    data$error <- (wrap(response - target))/2
    ...
    if(!is.null(id_var)){
      final_data <- data %>%
        ...
        summarise(mean_absolute_error =
                  ...
                  std = get_precision_single(.data$error)[, 3])
    } else{
      final_data <- data %>%
        summarise(mean_absolute_error =
                    get_mean_absolute_error(.data$error),
...
get_precision_single <- function(error, target = 0) {
  ...
  precision <- (1 / cstd(error))
  std <- (cstd(error))
  # Bias
  bias <- cmean(error)

  return(data.frame(precision = precision, bias = bias, std = std))
}


get_std_single <- function(error, target = 0) {
