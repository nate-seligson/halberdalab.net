...
function generateArrayMaxMinSD(setSizeInput,minDistance, maxRelDistance, MaxDistance, minRange, maxRange, minSD,maxSD) {
    ...
    while (flagTest == 'bad') {
        ...
        if  (Math.abs(Math.max(...Myarray) - Math.min(...Myarray)) > MaxDistance) {
            flagTest = 'bad';
       // } else if (getStandardDeviation(Myarray) < minSD || getStandardDeviation(Myarray) > maxSD) {
       //     flagTest = 'bad';
        } else if (Myarray.length == (setSizeInput) && getStandardDeviation(Myarray) < minSD) { 
            flagTest = 'bad';

        } else if (Myarray.length == (setSizeInput) && getStandardDeviation(Myarray) > maxSD) { 
            flagTest = 'bad';
    
        } else {
            flagTest = 'good';
            console.log(getStandardDeviation(Myarray))

        }
          //  getStandardDeviation(Myarray)
    ...
    //  if (flagTest = 'good' && getStandardDeviation(Myarray) > minSD && getStandardDeviation(Myarray) < maxSD) {
    //    flagTest = 'good'
    //    console.log(getStandardDeviation(Myarray))
    //  } else {
    //    flagTest = 'bad'
    //  }
        ...
        for(let kk = 0; kk < Myarray.length; kk++) {
            if (Myarray[kk] > 359) {
                ...
                var sd = getStandardDeviation(Myarray);
              //  console.log(sd)
